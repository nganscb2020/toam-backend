...
// Cho phép chèn ảnh vào GIỮA nội dung bài viết bằng cú pháp "![mô tả](/uploads/xxx.jpg)"
// (giống Markdown, nhưng CHỈ nhận diện đúng cú pháp ảnh, không phải Markdown đầy đủ).
// CHỈ chấp nhận đường dẫn "/uploads/..." do chính hệ thống tải lên — không cho nhúng ảnh
// hay bất cứ thứ gì từ nơi khác, để không ai lợi dụng ô nội dung nhét mã độc/link lạ vào.
// PHẢI khớp với hàm cùng tên trong public/js/safe.js (phía trình duyệt).
...
function renderArticleContent(raw) {
  ...
  let out = '', last = 0, m;
  while ((m = INLINE_IMAGE_RE.exec(text))) {
    out += esc(text.slice(last, m.index));
    out += `<img src="${esc(m[2])}" alt="${esc(m[1])}" loading="lazy">`;
    last = INLINE_IMAGE_RE.lastIndex;
  }
